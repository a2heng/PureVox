...
## 2026-09-26 — 新增 PureVox Web：Lite 两档的浏览器单 HTML 版

- **新增 Web 版（浏览器内跑同一套降噪）**：把 Lite 两个最简档移植成浏览器页面，
  产物各是一个**自包含单 HTML**——CSS / JS / AudioWorklet / Worker 源码全部内联，
  ONNX Runtime Web 运行时与降噪模型一律 **base64** 注入，页面零外部请求、断网可用。
  - `purevox-web-mic.html`：对应 lite_mic（麦克风 → 降噪 → 本地输出）
  - `purevox-web-net.html`：对应 lite_net（网络输入 → 降噪 → 本地输出）
- **模型契约与桌面端完全一致**：波形 hop 进出（480 = 10ms @48k）、STFT 在模型图内、
  enh_hop 滞后 1 hop、cache 形状按模型输入自适应；默认档仍是 202609b（中号），
  构建时 `--model a|b|c` 换档。实测浏览器内单跳推理约 1.5ms（预算 10ms）。
- **数据面与桌面端同构**：采集（重采样到 48k、按 hop 切帧）→ 前增益 → ONNX 降噪
  → 后增益 → 播放环（预缓冲 5 hop、欠载静音重同步、封顶 30 hop 丢最旧）。
  浏览器自带的 AEC / 降噪 / AGC 一律关闭——降噪只有 PureVox 一条实现路径。
- **Net 版的「网络输入」走 WebRTC**：浏览器不能监听端口，故用 WebRTC 代替 WSS，
  收发两端在同一页面内切换，靠手工交换连接码（deflate + base64）建连，
  **不用 STUN/TURN、不用服务端**，只走 host candidate。**两条路径分清**：程序资源
  （页面 / ONNX 运行时 / 模型）从 Pages 或 CDN 加载（可以走外网），而**音频只在
  局域网内直传**，不经任何服务器、不出局域网（两端需同一网段）。降噪只在接收端做。
- **两端控件各归其位**：发送端只有一个麦克风输入（原样送上网，不输出、不降噪，
  输出行与前后增益整组隐藏）；接收端是扬声器输出 + 前后增益，增益只在接收端调。
- **修「连上了却没声音」**：浏览器只在有用户激活时放行音频，而用户激活约 5 秒就过期；
  网络版是「点连接 → ICE 若干秒后音轨才到」，此前那一刻才建 AudioContext，会被自动
  播放策略挂起——连接正常、有数据在跑，但一点声音没有且不报错。现改为点「连接」时
  就把音频上下文建好拉起，并在首次点页面/页面回前台时兜底恢复；调试面板新增「音频」
  一栏直接显示 `running`/`suspended`，被挂起时日志也会提示。
- **发送端不再是黑盒**：此前发送端不跑降噪链路，调试面板与电平表全空，看起来像没工作。
  现按角色分栏——接收端看 RTT/丢包/抖动，发送端看协商状态、已发送字节、麦克风，
  并给发送端补了电平表（独立 AudioContext 只连 Analyser，不回放自己）。
- **修发送端主按钮误拆连接**：生成应答码后没有置运行态，再点一次会重走一遍生成流程，
  把已建好的连接拆掉；现在第二次点击是挂断。应答失败时也会释放麦克风。
- **修重复应答**：重复接受同一份应答码会抛浏览器原始错误（`wrong state: stable`），
  现按协商状态给出可照做的提示；生成新邀请码或断开连接时自动作废旧应答码。
- **新增 `purevox-web/serve.py`**：本地 HTTPS 伺服（默认 59124）。浏览器只在
  **安全上下文**（HTTPS 或 localhost）里给麦克风权限、才允许 WebRTC，因此不能
  直接双击 HTML。证书复用桌面端那份 PureVox Local CA（`~/.purevox/ca/`），
  整个产品只需信任一次自签证书；手机与电脑同网段都能打开。
- **产物是瘦页面 + 共享重资源**（面向 GitHub Pages 部署）：页面只含应用代码
  （约 76/88 KB），ORT 运行时与降噪模型按 URL 加载、**两个页面共用同一份**，
  URL 稳定不带版本串（靠 HTTP 缓存长期有效）。首屏传输从 30.5MB 降到 22.9MB，
  之后每次打开只传页面本体几十 KB。想换 CDN 用 `--base-url <前缀>` 即可。
- **构建与产物**：`python purevox-web/build/build_web.py` 打包（默认 b 档）；
  产物在 `purevox-web/dist/`（页面 + `assets/` + `build.json` 清单）、第三方运行时
  在 `purevox-web/vendor/`，两者均不入版本库，源码与脚本入库。

## 2026-09-26 — 新增「AI 降噪 · 旧版 v6」节点（202606）

- **新增第四档降噪**：把 lightweight-denoise-48k 的 v6 权重（`checkpoint_epoch_14.tar`，
  0.52M）重新导出成 PureVox 契约，接进插件目录——「**AI 降噪 · 旧版 v6**」
  （节点 `denoiser_v6`）。用于与 202609 三代做对比和回退，默认档位不变
  （仍是中号 202609b）。
- **插件目录顺序调整为 小号 → 中号 → 大号 → 旧版 v6**（原为 中号 → 小号 → 大号）。
  节点名 `denoiser_s` / `denoiser_m` / `denoiser_l` 全部不变，旧配置继续可用，
  仅目录展示顺序变化。
- **202606 模型按 202609 契约重新导出**（`models/purevox_denoise_202606_ep0014_op17.onnx`，
  opset 17）：波形 hop 进出、**傅里叶内化在模型图内**、enh_hop 滞后 1 hop、
  2 进 2 出 + 单个扁平 cache（15176）。引擎与 UI 零改动复用——引擎按模型
  `cache_in` 形状自适应，四档可任意热切换。
- 与 202609 的唯一语义差别：202606 的分析/合成窗是**普通 Hann + 常量 ola 包络**
  （v6 训练时就是如此，换 sqrt-Hann 会让它的 log10 幅值特征失真），202609 用
  sqrt-Hann。两者对引擎透明。

## 2026-09-25 — 降噪三档占位：小号 a / 中号 b / 大号 c（训练中）

- **降噪三档接进应用**：把训练侧 `202609a/b/c` 按**小号 / 中号 / 大号**三档接进插件
  目录——「AI 降噪 · 小号」（202609a，0.28M）、「AI 降噪 · 中号」（202609b，0.57M，
  默认）、「AI 降噪 · **大号**」（202609c，1.66M，新增节点 `denoiser_l`）。中号/小号的
  节点名 `denoiser_m` / `denoiser_s` 保持不变（旧配置继续可用），只换模型与 key。
- ⚠️ **三份 ONNX 是训练中期的占位权重**（a ep0278 / b ep0046 / c ep0012），目的只是先
  把三档链路、接口与热切换跑通。**模型权重随后续训练滚动替换，但接口契约不变**：hop
  480、enh_hop 滞后 1 hop、扁平 cache 按形状自适应（11,162 / 15,306 / 36,506）。换权重
  时只需替换 `models/` 里的文件并更新 `model_config.py` 的文件名，引擎与 UI 无需改动。
- 移除旧的 `202609m` / `202609s` 两个模型文件与注册项。

## 2026-09-24 — 输出采样率自适应（输入输出全适配，无门禁）

- **输出任意采样率自动适配**：播放设备按原生采样率打开（如 44100Hz），回调
  经输出适配器把 48kHz 引擎帧重采样到设备域（逐回调精确帧数）。此前 44.1k
  输出硬开 48k 会报 `-9997` 弹框阻止——该门禁与弹框已整体删除，启动不再拦截。
- **AEC 延迟校准播放端同自适应**：探测音按目标扬声器原生采样率重采样后播放，
  44.1k 扬声器可直接校准；所选端点打不开即明确失败，不再静默改播默认输出
  （那曾是"播错端点导致校准失败"的根因）。
- **状态栏追加输出信息**：如「输出：2ch 48kHz → 44100Hz（自适应重采样）」。
