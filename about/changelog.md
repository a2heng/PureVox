...
## 2026-09-24 — Windows 本地输入采样率自适应

- **输入任意采样率自动适配**：本地输入（录音输入 / AEC 输入的麦克风 / AEC far
  选麦克风）按设备原生采样率与声道打开（如 2 声道 44100Hz），自动下混单声道
  并重采样到 48kHz 再处理，不再弹框阻止启动。输出设备仍强制 48kHz（WASAPI
  严格 / MME 宽松不变）。
- **状态栏新增一行采样率信息**：启动后显示如「输入：2ch 44100Hz →
  48kHz（自适应重采样）」或「输入：48kHz（直通）」，AEC 行另附 far 端适配状态。
- **AEC 延迟校准同自适应**：校准录音按麦克风原生采样率/声道采集后转 48k
  再互相关，44.1k 麦克风可直接校准（播放端仍走 48k 输出）。

## 2026-09-24 — 降噪模型换代：中号 / 小号两个节点

- **降噪模型换代**：主降噪换成新一代 v9 主干（FreqStem 可学习频带 + 6 层 U-Net），
  并拆成两个独立节点——**「AI 降噪 · 中号」（202609m，0.62M，默认）** 与
  **「AI 降噪 · 小号」（202609s，0.31M，更快）**，按需加入插件链（两者可同时存在于
  链中，引擎各自独立加载）。旧降噪节点 `denoiser` 与旧模型
  `purevox_denoise_202609_ep0000.onnx` 已移除：旧配置中的 `denoiser` 项会被跳过，
  重新从插件列表添加中号或小号即可。Lite（lite_mic / lite_net）只用中号。
- **降噪节点使用建议**：先把前增益加到足够大，小声音才不会被模型误清；若增益后
  声音过大，再在后方接一段负增益即可（中号/小号节点下方已附该提示）。

## 2026-09-15 — Linux 发行包修复 + 提示与文档校正

- **Linux 发行包修复**：`qr_tk.py`（网络输入二维码渲染）与 `qrcode` 依赖此前未随
  deb/rpm/AppImage 打包，网络输入节点的二维码缺失/不可用；现补入发行 payload 与
  Linux 依赖清单。deb/rpm 的宿主依赖改为显式声明 `pipewire-pulse` 与 `libpulse`
  （原先只声明 `pipewire`，而该包并不拉入二者）。
- **48kHz 启动弹框修复**：检测失败时弹框详情与标题重复（分隔符不一致导致切分失效）；
  标题与提示统一为「以下设备不支持 48kHz，已阻止启动」。
- **失败提示更明确**：二维码不可用会注明原因（缺 qrcode 库 / 数据无效）；TSE 参考
  录音未捕获到音频、参考加载失败、无可用音频后端、网络服务器启动失败均补充原因或
  排查方向。
- **浏览器端**：HTTPS 下连接失败时也会显示自签证书信任指引（此前该分支不可达）。
- **Android**：调试面板 `backlog` 不再显示伪造的 0，改为 `--`。
- **文档校正**：README（中/英）与 Windows/Linux 使用手册对齐当前节点式 UI，移除已
  废弃的「音频接口 / 监听 / 五种模式」说明及 `requirements.txt`、`libpipewire-0.3-devel`
  等过时命令；修正英文 README 的编码损坏；关于页第三方库清单补 `av` / `qrcode`。

## 2026-09-15 — 主线网络输入节点看齐 Lite Net（网卡 / 二维码 / 状态）

- **「网络输入」节点改为与 PureVox Net Lite 同构**：不再是一个手填推流地址框，而是
  **网卡（IP）下拉 + 推流页二维码 + 服务状态**（端口 / 活跃客户端数，运行中每秒刷新）。
  推流地址由所选网卡 IP + 服务器端口推导（写入节点 url，仍是启动前提，也是二维码内容）；
  切换网卡即热更新地址并让服务器按新网卡重签证书、重注册 mDNS。
- **统一代码**：二维码渲染（qrcode 矩阵 → Tk PhotoImage，零 PIL）抽为主线与 Lite 共用的
  `qr_tk.make_qr_photo`；本机网卡枚举（带网卡名、物理口优先、TUN/VPN 沉底）收敛到平台层
  `pvplatform.netinfo`，主线网络节点与 Lite Net 页共用同一份实现。
...
- **节点更名**（菜单与行标题同源 = spec.label）：音频输入 → **录音输入**，
  远程推流输入 → **网络输入**，桌面输入 → **播放输入**，回声消除 AEC →
  **AEC 输入**，音频输出 → **音频输出**；相关告警文案同步。
- **修复「只加播放输入没声音」**：引擎把「无麦克风输入」一律当纯媒体会话
  （静音直出），回环输入行从不被拉取——现在回环输入也算设备输入，只有
  「无麦克风 + 无回环 + 无网络」才走纯媒体会话。
- **修复播放输入「卡卡卡」**：① 回环行按 hop 消费而生产≈消费速率，任何抖动
  都让 pop 落空并插入静音——改为先预充 3 个 hop 再消费，吸收包到达抖动；
  ② 混音归一化分母改用「配好的输入路数」（固定），不再随某路本 hop 有无
  数据在 1 与 1/N 间跳变（拉链噪声）；③ WASAPI 回环读取：一次取空排队包，
  并遵守 SILENT 标志（此前读未定义字节会爆音）。
- **修复播放输入轻微嘀嗒毛刺**：回环行不再按时间戳入网格（取整会让网格推进
  与样本数差 ±1，周期性丢/补样本），改按连续流推入——该行只需连续，不需
  跨时钟配对（配对仅 AEC far 需要）。
- **快捷键弹窗精简为两行**：`[☐]启停快捷键 + 录制框`、`[☐]启停提示音 +
  启动提示音 / 停止提示音 两个试听按钮`；去掉录制框的清除「✕」（开关即可），
  去掉重复说明与嵌套标题，新增 `hotkey_toggle_on` 开关。
- **方框按钮与配色微调**：`SquareButton` 外框底色即按钮底色（修「一开始很小、
  划过才正常」）；节点 `×` 红色、音效行 `×` 深棕；快捷键录制框加 1px 木色描边
  （否则白底融入浅色行）；主窗基准宽 400。
- **配色分层**：窗底更淡、插件行底色略深（`WINDOW #FFFCF2` / `PANEL #FFE9A8`），
  不用边框靠明度差分区。
- **音效行**：改用统一行项色（`ROW_ITEM #F6DE9C`，略暗于面板），行间留 1px 缝隙
  露出面板色作分隔（不再斑马纹）；行首复选框按标题拖拽手柄宽度缩进，与标题的
  复选框上下对齐。
- **VB-CABLE 卡（音频输出行内）**：大段文字改为**单向数据流向图示**（麦克风 →
  PureVox → CABLE In → CABLE Out → OBS/会议/语音等软件，实心三角箭头、左→右、
  随卡片宽度自适应并均摊余宽，使图示左右边缘与下方按钮框对齐）；三个按钮精简为
  「控制面板 / 驱动下载 / 视频教程」并前挂
  VB-CABLE 标签、等宽铺满一行；字号放大到正文、复选框统一 DarkCheck；虚拟声卡卡
  文案同步精简。
- **频谱配色**：底色由深棕改为「仅比卡片底色深一档」的浅暖黄（`#E0C88C` /
  空槽 `#EBD5A0`），并去掉画布上下的 2px 留边（原先上缘露出一条深色带）。
- **三色柱调深**：VU / 频谱的绿/黄/红三区中，黄、红各调深一档以贴近绿的明度
  （黄 `#FFB300`、红 `#E53935`），浅暖底上不再发飘；绿不变。
- **卡片内零留白**：标题行左右顶满卡片，`×` 正好落在卡片右上角；VU / 频谱画布
  顶满卡片四周（VU 底色改用面板色，与卡片融为一体）；卡片之间与外边距保留留白，
  相邻卡片上下间隙与左右留白同粗。

## 2026-09-15 — 音效板行内化：名称 / 快捷键 / 音量 一体

- **每个音效一行，一行全搞定**（从左到右）：**启用复选框** · **名称**（点击即
  播放一次）· **行内快捷键录制框** · **行内音量滑杆（±10dB，自适应占满剩余）**
  · 深棕 `×` 删除（行尾成列）。不再有节点级总音量、播放/停止钮，也没有
  「全部停止 / 清空快捷键 / 快捷键设置」等额外按钮。
- **复选框＝该音效总开关**（等价于快捷键启停；未勾选则不绑快捷键），勾选态随
  音效一并持久化。快捷键点方框即录制，Delete 清除；清空录制内容的「✕」不再需要。
- 「设置 → 快捷键与提示音」弹窗精简为「启停热键 + 提示音」，窗口缩小（420×300）。
- **每个音效一套完整数据**（名称 / 库内路径 / 开关 / 快捷键 / 音量）整体存进
  `plugin_chain` → config.json；`×` 移除时同时丢弃 `~/.purevox/soundpad/` 内
  对应 WAV（仅当已无其他音效引用同一文件；外部路径永不删）。
- **修复「添加音效后新行不出现、移除也无效」**：音效列此前的增删作用在其副本上，
  `commit` 又回读原列表，导致改动被丢弃——现直接操作实列表。
- **修复方框按钮尺寸**：`SquareButton` 外框原先用宿主底色，静止时只见居中小字形、
  悬停重绘才显出整块方框（「一开始很小、划过才正常」）；现外框即按钮底色。
- 「＋ 添加音效」移到节点标题后面（行头）；节点 `×` 统一为红色方框、音效行 `×`
  为深棕方框（边长恒为 ctl_h，各节点右缘成列对齐）。
- 节点与菜单名去括号，显示为「音效板」；主窗基准宽 420 → 400。

## 2026-09-15 — 音效/音乐统一入库 ~/.purevox

- **媒体入库唯一实现**（`pvengine/components/audio_decode.py` 的 `import_media`）：
  添加音效或音乐时，任何来源文件一律归一为 **48kHz 单声道 16bit WAV**，分别存入
  `~/.purevox/soundpad/` 与 `~/.purevox/music/`（分开文件夹），配置只引用库内路径，
  不再依赖外部原始文件；miniaudio 不支持的容器/编码（m4a/mp4/aac/wma/opus…）
  用 PyAV 兜底解码后再入库。
- **库内文件名 `<原名>-<sha1 前 8 位>.wav`**：重复导入同一文件幂等复用，
  不同内容同名不互相覆盖；非法路径字符自动清洗。
- 旧的「选文件时就地生成 `<原名>.purevox.wav`」路径（`ensure_playable`）被
  上述入库路径取代并删除，不留平行实现。

## 2026-09-15 — 可变托盘图标 + 可录制全局快捷键 + 启停提示音

- **托盘图标随运行态变色**：停止=红色 P、运行=蓝色 P，提示气泡同步；右键菜单
  新增「启动 / 停止音频处理」项（打开 / 退出不变），左键仍切换主窗显隐。
- **全局快捷键重做（可自定义、可留空）**：新增「设置 → 快捷键与提示音」弹窗，
  点方框后按下组合键即录制，Delete 清除，清空 = 不监听。启停热键默认
  右 Alt + .（`hotkey_toggle`），旧固定绑定 Ctrl+Alt+1..9 与「快捷键开关」项
  一并移除（快捷键始终生效，配置键无迁移）。
- **音效板独立快捷键体系**：每个音效单独录制一个快捷键、按下即单次播放
  （连按回到开头重播，不叠加），不含任何自带绑定。
- **新增启停提示音**：6 套短促音色（柔和 / 清脆 / 气泡 / 电子 / 木鱼 / 风铃）
  可分别用于启动与停止，带独立复选框总开关（`cue_enabled`）、可试听；
  合成在 `pvengine.cues`（纯 numpy），
  播放送**系统默认输出设备**（Windows winsound / Linux pw-play），与音频处理的
  输出设备无关；停止提示音延后一拍并带失败重试，避免刚释放设备时被驱动拒播
  （修复「偶尔听不到」）。
- **热键被占用时提示**：录制到系统/其他程序已占用的组合会弹窗告知，便于换一组。
- **修复热键不生效**：热键线程回调原先经 `root.after` 跨线程投递主线程，实测静默
  丢失（Windows 上按热键无任何反应）——改为「线程安全队列 + 主线程轮询」，
  与托盘动作同一通路。

## 2026-09-12 — 频谱节点高度压缩

- `SpectrumCanvas` 请求高度 220 → 150（段数/频率范围/三色渲染均不变），
  减少频谱节点在面板里的纵向占用；数据与显示内容不变。

## 2026-09-12 — 修复 Linux 输出行隐藏虚拟卡后留空占位

- `NodeRow.ensure_body` 改为仅当参数区存在「已 pack 的可见子件」时才显示该区；
  此前 Linux「输出设备」行内嵌的虚拟声卡卡在输出切到非虚拟设备时只是
  `pack_forget` 卡片本身，参数区仍以空框占位。现切换设备时同步调用
  `ensure_body`，无可见内容即整体收起，不留空占位（卡片只应在选中
  `purevox_out` 时出现）。

## 2026-09-12 — 修复 Linux 列表滚轮方向相反

- 节点面板滚轮：`sys_wheel_delta` 在 X11（Button-4 上 / Button-5 下）返回的符号
  与 Windows（`e.delta`）相反，而调用方又统一 `yview_scroll(-d)` 取反，导致
  **Linux 下滚轮方向与 Windows 相反**。现统一为「负=上/前，正=下/后」并直接传给
  `yview_scroll`，两平台方向一致；同时兼容非 Windows 的 `<MouseWheel>` delta。
  （设备下拉弹层原本就正确，未改。）

## 2026-09-12 — 频谱改为 64 段 Mel（20Hz–16kHz）+ 连续槽位

- **引擎 Mel 滤波器组**：128 段 / 20Hz–20kHz → **64 段 / 20Hz–16kHz**
  （`pvengine/dsp/spectrum.py`）；
- **UI 频谱连续槽位**：改为 64 段全画；每段先铺满底槽（静音段也有），不再因
  静音 `continue` 跳过而留下空洞；段间隙仅在列宽足够时留 1px、窄时贴合，
  槽位连续不断裂；`_hz_to_frac` 同步到 16kHz；
- **频谱底反差 + 三色柱**：频谱底改为深暖棕 `SPEC_BG`、空槽 `SPEC_SLOT`（衬托
  彩色柱）；每段按高度分绿(-90..-45) / 黄(-45..-30) / 红(-30..-20) 三区，
  每段最多 3 个矩形叠画（非逐像素，省计算）；输入高于输出时上方叠蓝灰
  噪声残留段。

## 2026-09-12 — UI 主题一致性：插件行去白色割裂 + 可视化底色统一

- **插件节点行统一底色**：参数区/卡片/音效板/音乐播放器/网络输入说明等此前用
  纯白 `BASE`，在面板行下方形成一条条白色割裂带；现全部随行底色 `PANEL`，
  整行统一；
- **减少白色碎块**：复选框未勾选、设备下拉框及其弹层由纯白改为羊皮纸 `WINDOW`，
  边框/悬停色不变；插件面不再出现零散白块；
- **可视化底色令牌化**：新增 `theme.VIZ_BG`（暖白），`VUCanvas` / `SpectrumCanvas`
  不再继承父底色（此前父底色是白/黄不定，忽白忽黄），统一用令牌；
- 仅输入控件（网络输入「推流地址」Entry）保留纯白。

## 2026-09-12 — AGC 重写：峰值包络 + 非对称弹道（消除抽吸 / 底噪抬高）

- 原 AGC 每 10ms 帧按峰值**瞬时**跳到目标（无 attack/release），人声峰值起伏导致
  增益来回跳（抽吸/喘息）；静音门仅 -60dBFS，词间底噪被抬高达 +30dB，忽大忽小；
-   现改为峰值包络（attack ~3ms / release ~80ms）+ 非对称增益弹道（降增益 ~4ms 防削波、
  升增益 ~300ms 防抽吸）；目标 -12dBFS；静音门 ~-55dBFS 之下不补增益并缓慢回落 unity，
  避免抬高底噪；soft clip 保留；
- **新增「最大增益 dB」滑杆（0~30，默认 30）**：手动限制自动提升上限
  （0 = 只衰减不提升），实时生效；衰减下限固定 -30dB；
- **AGC 行布局**：标题行显示「标题 + 增益/峰值数据」（数据紧贴标题、定宽防实时
  刷新抖动），最大增益滑杆移到下方参数区，不再与标题挤在同一行；
- 新增 `tests/test_agc.py`（合成 DSP：稳态收敛 / 静音不补 / 低于门限不补 /
  ±30dB 限幅 / 最大增益滑杆 / 回落 unity / 不抽吸 / soft clip 有界）。
